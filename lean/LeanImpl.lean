...
- There may be other versions of this code that specialize it and make it fast.
-/
...
Combines token embeddings and positional embeddings for each position in the input.

@param inp An array of shape (B,T) containing token ids for each position in inp[b, t]
@param wte Token embeddings of shape (V,C). Short for "weight token embeddings"
@param wpe Positional embeddings of shape (maxT,C). Short for "weight positional embedding"
@param B Batch size
@param T Sequence length
@param C Number of channels

@return An array of shape (B,T,C) containing the combined embeddings
-/
def encoder_forward (inp : Array Int) (wte wpe : Array Float) (B T C : Nat) : Array Float := Id.run do
  -- inp: (B,T)
  -- wte: (V,C)
  -- wpe: (T,C)
  let mut out := Array.mkArray (B*T*C) 0.0  -- out: (B,T,C)
  for b in [:B] do
    for t in [:T] do
      let ix := inp[b * T + t]!  -- ix: scalar
      for i in [:C] do
        let outIdx := b * T * C + t * C + i  -- outIdx: scalar
        let wteIdx := ix * C + i  -- wteIdx: scalar
        let wpeIdx := t * C + i  -- wpeIdx: scalar
        out := out.set! outIdx (wte.get! wteIdx.toNat + wpe.get! wpeIdx)  -- out[outIdx]: scalar
  out  -- out: (B,T,C)

/--
Test for the encoder_forward function
-/
def test_encoder_forward : IO Unit := do
  -- Test case setup
  let B := 2  -- Batch size
  let T := 3  -- Sequence length
  let C := 4  -- Number of channels
  let V := 5  -- Vocabulary size

  -- Create input token ids
  let inp := #[0, 1, 2, 3, 4, 5]  -- inp: (B,T) = (2,3)

  -- Create token embeddings (wte)
  let wte := Array.mkArray (V*C) 1.0  -- wte: (V,C) = (5,4)

  -- Create positional embeddings (wpe)
  let wpe := Array.mkArray (T*C) 0.2  -- wpe: (T,C) = (3,4)

  -- Call the encoder_forward function
  let result := encoder_forward inp wte wpe B T C  -- result: (B,T,C) = (2,3,4)

  -- Expected output calculation
  let expected := Array.mkArray (B*T*C) 1.1  -- expected: (B,T,C) = (2,3,4)

  -- Assert that the result matches the expected output
  dbg_trace "result: {result}"
  dbg_trace "expected: {expected}"

  IO.println "encoder_forward test passed!"

#eval  test_encoder_forward
